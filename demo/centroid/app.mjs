...
function render() {
  ...
  $('minimum-status').textContent = atMean
    ? '代表点が重心に一致しました。ここで2乗誤差が最小になります。'
    : '重心から離れている分だけ、最小値より大きくなっています。';
  $('discovery').textContent = revealed
    ? (atMean ? 'ここから少し動かして、誤差が増えることを確かめよう。確かめたら、下の説明へ進みましょう。' : 'ピンクのひし形が重心です。青い代表点を近づけて、誤差の変化を見てみよう。')
    : 'まずは青い点を動かし、誤差が小さくなる場所を探そう。見つけたら、重心と比べてみましょう。';
  renderHistory();
}
